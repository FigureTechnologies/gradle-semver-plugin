...
fun Version.nextVersion(
    providedStage: Stage,
    providedModifier: Modifier,
): Version =
    when {
...
private fun Version.nextStable(incrementer: Inc): Version = inc(incrementer)

private fun Version.nextStableWithPreRelease(
    incrementer: Inc,
    preRelease: String?,
): Version = inc(incrementer, preRelease)

private fun Version.nextSnapshot(incrementer: Inc): Version = inc(incrementer, Stage.Snapshot.value)

private fun Version.newPreRelease(
    incrementer: Inc,
    stage: Stage,
): Version = inc(incrementer, "${stage.value}.1")
